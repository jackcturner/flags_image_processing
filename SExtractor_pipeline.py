...
class SExtractor():
    ...
    def __init__(self, config_file, sexpath = 'sex'):
        ...
        # Read the SE configuration file and split into SE and genral config parts.
    ...
    def write_params(self, params, outdir):
        """Write output parameters in a list to a text file 
    ...
    def convert_to_flux(self, cat, config):
        """Convert counts in a catalogue to flux.
    ...
    def convert_to_hdf5(self, catalogue, config):
        ...
        config (dict)
            Config dictonary containing all parameters
             to be added as attributes.
    ...
    def get_aperture_config(self, SEconfig):
        """Return an updated config file with parameters appropriate for measuring in apertures around set locations.
        
        Arguments
        ---------
        config (dict)
            Dictonary containing the config to be updated.
    ...
    def get_aperture_locations(self, sci, hdr, mask, radius, napers = 10000, overlap = False, overlap_maxiters = 50000, outname = 'aperture_image.fits'):
        """Create a detection image with value one at random aperture centres.
        
        Arguments
        ---------
        sci (2D array)
            The science image in which to place the apertures.
        hdr (astropy.io.fits.Header)
            The header containing image WCS information.
        mask (2D array)
            Science image source mask.
        radius (float)
        ...
        depth = ImageDepth(radius, nsigma=1.0, napers=napers, niters =1, overlap=False, overlap_maxiters=overlap_maxiters)
    ...
    def measure_depth(self, sci_filename, err_filename, mask_filename, radius, psf_filename, min_apers = 1, max_apers = 50, max_iters = 50000, bins = 20, outdir = ''):
        """Use randomly placed apertures to measure the 5-sigma depth and noise distribution of an image.
        
        Arguments
        ---------
        sci_filename (str)
            Path to sicence image fits file.
        err_filename (str)
            Path to error image fits file.
        mask_filename (str)
            Path to source mask fits file.
        radius (float)
            Radius of the random apertures to use in pixels.
        psf_filename (str)
            Path to PSF fits file.
        min_apers (int)
            The number of apertures to place.
        max_apers (int)
            The maximum number of apertures to place.
        max_iters (int)
        ...
        self.get_aperture_locations(sci, hdr, mask, radius, max_apers, overlap_maxiters=max_iters, outname=det_filename)

        # Generate default SExtractor config file.
        sexfile = self.generate_default(outdir)

		# Build the base SE command line argument.
        detcmd = [self.sexpath, "-c", sexfile, det_filename, sci_filename,'-WEIGHT_IMAGE', err_filename]
        ...
        radii, cog, p = measure_curve_of_growth(psf, position=None, radii=None, rnorm='auto', nradii=30, rbackg=False, showme=False, verbose=False)
        f = lambda r: np.interp(r, radii, cog)

        # Correct by the fraction of the PSF enclosed within the aperture used and convert to 5 sigma.
        mad *= 5/f(radius)

        # Assuming units of nJy, calculate the 5 sigma limit in magnitudes.
    ...
    def measure_uncertainty(self, sci_filename, err_filename, seg_filename, SEconfig, config, outdir):
        ...
        if err_config['RADII_SPACING'] == 'linear':
            radii = np.linspace(err_config['MIN_RADIUS'], err_config['MAX_RADIUS'], err_config['N_RADII'])
        else:
            radii = np.logspace(np.log10(err_config['MIN_RADIUS']), np.log10(err_config['MAX_RADIUS']), err_config['N_RADII'])
        ...
        self.get_aperture_locations(sci, hdr, mask, max(radii[smaller]), err_config['N_SMALL'], overlap_maxiters=err_config['MAX_ITERS'], outname=small_filename)
 
        large_filename = f'{outdir}/large_apertures.temp.fits'
        self.get_aperture_locations(sci, hdr, mask, max(radii), err_config['N_LARGE'], overlap_maxiters=err_config['MAX_ITERS'], outname=large_filename)

        # Generate a default configuration file.
        sexfile = self.generate_default(outdir)

		# Build the base command line arguments for small and large aperture runs.
        ...
        sig1 = sigma_clipped_stats(sci, mask)[2]    # The sigma-clipped standard deviation of all non-object pixels.
        ...
        # The percentage error to use when fitting. Can help weight low or high regions.
        ...
        p0 = [initial + 1e-7 * np.random.randn(len(initial)) for i in range(err_config['WALKERS'])] # Step methodology.
        ...
        with np.errstate(invalid='ignore'):
            ...
            for column in cat.colnames:
                if column == 'FLUX_AUTO':
                    ...
                    # Extract the corresponding noise predicted by the fit and scale based on local value.
            ...
            # Overwrite the old catalogue with this one containing the new uncertainties.
        ...
        if err_config['SAVE_FIG'] == True:
            ...
            plt.title(f'{os.path.splitext(os.path.basename(SEconfig["CATALOG_NAME"]))[0].split(".cat")[0]}', fontsize = 10)
            plt.minorticks_on()
    ...
    def SExtract(self, image, weight = None, parameters = {}, measurement = ['FLUX_AUTO'], outdir = None):
        ...
        measurement (list)
            List of SE outputs to save.
        outdir (None, str)
            Directory in which to store outputs. If None, create "output" directory.
        """
        ...
        if type(image) == list:
            img_SEconfig['CATALOG_NAME'] = f'{outdir}/{os.path.splitext(os.path.basename(image[1]))[0]}.cat'
        else:
            img_SEconfig['CATALOG_NAME'] = f'{outdir}/{os.path.splitext(os.path.basename(image))[0]}.cat'
        ...
        if img_config['EMPIRICAL'] == True:
            ...
            if img_SEconfig['CHECKIMAGE_TYPE'] != 'SEGMENTATION':
                ...
                img_SEconfig['CHECKIMAGE_NAME'] = f'{img_SEconfig["CATALOG_NAME"][:-4]}.temp_seg.fits'
        ...
        if img_config['EMPIRICAL'] == True:
            if (type(image) == str):
                self.measure_uncertainty(sci_filename=image, err_filename=weight, seg_filename=img_SEconfig['CHECKIMAGE_NAME'], SEconfig=img_SEconfig, config=img_config, outdir=outdir)
            else:
                self.measure_uncertainty(sci_filename=image[1], err_filename=weight[1], seg_filename=img_SEconfig['CHECKIMAGE_NAME'], SEconfig=img_SEconfig, config = img_config, outdir=outdir)
